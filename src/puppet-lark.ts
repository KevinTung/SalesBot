...
axinstance.interceptors.response.use(res => {
  console.log('Previous Request',res.request._header)
  return res;
...
class PuppetLark extends Puppet {
  ...
  async start (): Promise<void> {
    ...
    // this.app.use(router); // notice how the router goes after the logger.
    ...
    this.app.post('/', async (req: any, res: any) => {
      ...
      if (payload.type === 'url_verification') {
        ...
        console.log(res.status(200).json({ challenge: payload.challenge }))
        return null
        //1. print the json, see where goes wrong
        //2. check new version of challenge api
  ...
  async messageSendXLSFile (conversationId: string, file: FileBox, file_name:string): Promise<string | void> {
    //const _mimeType = file.mimeType ? file.mimeType : ''

    const token = await this.getTenantAccessToken(this.appId, this.appSecret)
    const imageKey = await this.uploadXLSFile(token, file, file_name)
    ...
    const params = new URLSearchParams({
        ...
        params
    // const response = await axinstance({
    //   data:
    //   {
    ...
    //   },
    // })
    const response = await axinstance.post('https://open.feishu.cn/open-apis/im/v1/messages',
      {
        receive_id: conversationId,
        content: "{\"file_key\":\""+imageKey.toString()+"\"}",
        msg_type: 'file',
      },{
      headers: {
        Authorization: 'Bearer ' + token,
    ...
    if (response.data.code === '0') {
      log.verbose('PuppetLark', 'messageSendXLSFile', 'Successfully send image.')
    } else {
      log.error('Failed to send XLS File:', response.data.msg)
    }
  }
  ...
  private async uploadXLSFile (_token: string, image: FileBox, file_name: string): Promise<string> {
    const _image = await image.toStream()
    const formData = new FormData()
